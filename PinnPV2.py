...
class PID():
    def __init__(self, path=None, date=None, index=None, **kwargs):
        ...
        if index is None:
            self.fnamevar_path = "PI_variables_2obs" + self.date + ".dat"
        else:
            self.fnamevar_path = "PI_variables_2obs" + self.date + "_" + str(index) + ".dat"
    ...
    def build(self, transform=None, **kwargs):
        ...
        if transform is not None:
            ...
            def output_transform(t, y):
                ...
                return linear + factor * torch.Tensor([1, 1, 1, 1, 1]) * y
...
if __name__ == "__main__":
    ...
    microgird.build(transform='all',
                    net=dde.nn.FNN([6] + [100] * 3 + [5], "tanh", "Glorot uniform"),
                    lr=1e-3,
                    iterations=3000000)
