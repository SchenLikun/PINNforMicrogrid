...
class Load():
    def __init__(self, path=None, date=None, index=None, **kwargs):
        ...
        self.variable_list = [Pzip, Qzip, alpha_1, alpha_2, alpha_4, alpha_5, a, b, c, Hm, xr, xs, rr, xmu, rs, R_line,
                              X_line]
    ...
    def rename_csv(self):
        ...
        self.input_data.rename(
            columns=lambda x: dict_col[x.split('|')[-1]] if x.split('|')[-1] in dict_col.keys() else x, inplace=True)
    ...
    def build(self, transform=None, **kwargs):
        ...
        data = dde.data.PDE(
            geom,
            self.ode,
            [observe_y0, observe_y1, observe_y1, observe_y2, observe_y3, observe_y4, observe_y5, observe_y6, observe_y7,
             observe_y8, observe_y9, observe_y10, observe_y11, observe_y12, observe_y13, observe_y14, observe_y15],
            # [],
            anchors=observe_t,
        )
...
if __name__ == "__main__":
    ...
    microgird = Load(path=input_data_path,
                     st=0.02,
                     et=0.04,
                     index='normal_Small_Domain_hard_cons'
                     )
    ...
    microgird.build(transform='all',
                    net=dde.nn.FNN([6] + [256] * 3 + [21], "tanh", "Glorot uniform"),
                    lr=1e-6,
                    iterations=5000000)
