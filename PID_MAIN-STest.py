...
class PID():
    ...
    def build(self, transform=None, **kwargs):
        ...
        '''
        这里先目前假设的只能观测到工程实际值, 如果运行失败再考虑增加其他学术理论观测值，比如Iqref， DeltaIq的初始值 / 全部值
        '''
        ...
        if transform is not None:
            ...
            '''
            这里是输入转换: Input transform (Taylor series expansion)
            '''

            def feature_transform(t):
                t_expanded = t.clone()
                for n in range(1, 6):
                    term = ((-1) ** n) * (t ** (2 * n + 1)) / torch.tensor(float(np.math.factorial(2 * n + 1)))
                    t_expanded = torch.concat((t_expanded, term), axis=1)
                return t_expanded
