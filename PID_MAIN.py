...
class PID():
    def __init__(self, path=None, date=None, index=None, **kwargs):
        ...
        if index is None:
            self.fnamevar_path = "PI_variables_ModelSave" + self.date + ".dat"
        else:
            self.fnamevar_path = "PI_variables_ModelSave" + self.date + "_" + str(index) + ".dat"
        ...
        def PI_ode(x, y):
            ...
            '''
            return [
                -(PQkp * dI_dt1 + DeltaQ * (PQki*10)) - dO_dt1,
    ...
    def build(self, transform=None, **kwargs):
        ...
        observe_y1_0 = dde.icbc.PointSetBC(observe_t[:5], y[:5, 1:2], component=1)
        observe_y1_1 = dde.icbc.PointSetBC(observe_t[-5:], y[-5:, 1:2], component=1)
