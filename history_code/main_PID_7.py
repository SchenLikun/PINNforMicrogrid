...
if __name__ == "__main__":
    k_p_ = dde.Variable(1.0)
    k_i_ = dde.Variable(1.0)
    input_variable = [k_p_, k_i_]

    # err = pd.read_csv('data//pi/3phaseFLTKp0.5Ki5Step0.1No.0.csv')['err'].to_numpy().reshape(-1, 1)
    ...
    def Microgrid_system(x, y):
        ...
        k_p = (torch.tanh(k_p_) + 1) * 0.125 + 0.125
        k_i = (torch.tanh(k_i_) + 1) * 0.25 + 0.25

        dU_dt = dde.grad.jacobian(y, x, i=0)
        dI_dt = dde.grad.jacobian(y, x, i=1)
        
        return [
            (k_p*dI_dt/10+I*0.5) - dU_dt
            # - torch.as_tensor(err)
        ]

    input_data_path = '../data/pi/3phaseFLTKp0.5Ki5Step0.1No.0.csv'
    data_col_name = ['Time', 'I', 'U']
    real_value_list = [0.5, 5]
    ...
    microgird = Microgird(path=input_data_path, 
                        ...
                        #   date='0929',
                          index='PID_base',
                          net=dde.nn.FNN([6] + [100] * 3 + [2], "swish", "Glorot uniform"),
                          iterations=1000000
                          )
